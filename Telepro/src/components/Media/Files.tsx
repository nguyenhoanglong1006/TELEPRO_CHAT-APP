...
const Files: FC = () => {
  ...
  // lay data theo type = file tren firebase
    ...
    <div className="flex h-80 flex-col items-stretch gap-3 overflow-y-auto p-4">
      {data?.docs.map((file) => (
        <div key={file.id} className="flex items-center gap-4 p-2">
          <FileIcon
            className="text-dark-lighten h-6 w-6 object-cover"
            extension={file.data().file.name.split(".").slice(-1)[0]}
          />
          <div className="flex-grow">
            <h1 className="text-dark-lighten">{file.data()?.file?.name}</h1>
          ...
          >
            <i className="bx bxs-download text-dark-lighten text-2xl"></i>
          </a>
        </div>
      ))}
    </div>
