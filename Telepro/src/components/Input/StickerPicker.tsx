...
// get Sticker thuong su dung
const getRecentStickers = () => {
  const existing = localStorage.getItem("telepro-recent-stickers") || "[]";
  try {
...
const StickerPicker: FC<StickerPickerOpened> = ({ setIsOpened, onSelect }) => {
  ...
  const addRecentSticker = (url: string) => {
    ...
    localStorage.setItem("telepro-recent-stickers", JSON.stringify(added));
  ...
  return (
    <ClickAwayListener onClickAway={() => setIsOpened(false)}>
      {(ref) => (
        <div
          ref={ref}
          className="border-lighten bg-light absolute -left-16 bottom-full h-96 w-96 rounded-lg border-2 shadow-2xl"
        >
          {loading || error ? (
            ...
            <div className="flex h-full w-full flex-col">
              <div className="flex-grow overflow-y-auto p-3 pt-1">
                {/* Hien thi recent Sticker */}
                {recentStickers.length > 0 && (
                  <>
                    <h1 className="text-primary mt-2" id="sticker-recent">
                      Recent stickers
                    </h1>
                    <div className="grid w-full grid-cols-5 justify-between">
                      {recentStickers.map((url) => (
                        <SpriteRenderer
                          ...
                          className="hover:bg-lighten cursor-pointer"
                          src={url}
                          runOnHover
                        />
                      ))}
                ...
                {/* hien thi Collection stickers */}
                {data?.map((collection) => (
                  <Fragment key={collection.id}>
                    <h1
                      className="text-primary mt-2"
                      id={`sticker-${collection.id}`}
                    >
                      {collection.name}
                    </h1>
                    <div className="grid w-full grid-cols-5 justify-between">
              ...
              <div className="h-18 border-t-dark-lighten flex w-full flex-shrink-0 gap-2 overflow-x-auto border-t p-2">
                {recentStickers.length > 0 && (
                  ...
                  >
                    <i className="bx bx-time text-primary text-[26px] leading-[26px]"></i>
                  </button>
                )}
                {data?.map((collection) => (
                  <img
