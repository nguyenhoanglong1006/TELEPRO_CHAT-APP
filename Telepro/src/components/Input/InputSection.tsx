...
const InputSection: FC<InputSectionProps> = ({
  ...
  return (
    <>
      {fileDragging && (
        <div className="pointer-events-none fixed top-0 left-0 z-20 flex h-full w-full select-none items-center justify-center backdrop-blur-lg">
          <h1 className="text-3xl">Drop file to send</h1>
        </div>
      )}
      {previewFiles.length > 0 && (
        <div className="border-lighten flex h-32 items-center gap-2 border-t px-4">
      ...
      {previewFiles.length === 0 && !!replyInfo && (
        <div className="border-lighten bg-light flex h-[76px] justify-between border-t p-2">
          <div>
            <div className="flex items-center gap-2">
              <i className="bx bx-reply text-dark text-2xl"></i>
            ...
            {replyInfo.type === "text" ? (
              <p className="text-dark max-w-[calc(100vw-65px)] overflow-hidden text-ellipsis whitespace-nowrap md:max-w-[calc(100vw-500px)]">
                {replyInfo.content}
              </p>
          ...
          <button onClick={() => setReplyInfo && setReplyInfo(null)}>
            <i className="bx bx-x text-dark-lighten text-3xl"></i>
          </button>
        </div>
      )}
      <div
        className={`border-lighten bg-light flex h-16 items-stretch gap-1 border-t px-4 ${
        ...
        >
          <div className="relative flex flex-grow items-center">
            <input
              ...
              onPaste={handlePaste}
              className="bg-lighten border-lighten text-dark-lighten h-9 w-full rounded-full border p-2 pl-3 pr-10 outline-none transition duration-300 focus:border-gray-300"
              type="text"
              placeholder="Message..."
            />
            <button
