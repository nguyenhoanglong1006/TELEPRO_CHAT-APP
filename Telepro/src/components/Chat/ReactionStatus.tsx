...
// Badge sau khio reaction message
  ...
  return (
    <>
      <div
        ...
        className={`bg-dark-lighten border-dark-lighten absolute top-full flex -translate-y-1/2 cursor-pointer items-center gap-[2px] rounded-lg border px-2 text-sm ${
          position === "right"
            ? "right-8"
            : position === "left-tab"
            ? "left-[70px]"
      ...
      {isReactionStatusOpened && (
        ...
        >
          <div
            ...
            className="bg-light flex h-96 w-screen max-w-[400px] flex-col rounded-lg"
          >
            <div className="border-lighten flex flex-shrink-0 items-center border-b p-4">
              <div className="flex-1"></div>
              <div className="flex-1">
                <h1 className="text-dark text-center text-2xl">Reactions</h1>
              </div>
              <div className="flex flex-1 justify-end">
                <button
                  onClick={() => setIsReactionStatusOpened(false)}
                  className="bg-light flex h-8 w-8 items-center justify-center rounded-full transition duration-300 hover:brightness-125"
                >
                  <i className="bx bx-x text-dark text-3xl"></i>
                </button>
              </div>
            </div>
