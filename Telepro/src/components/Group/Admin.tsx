        ...
        .map((user) => (
          <div key={user.uid} className="flex items-center gap-3 px-4">
            <img
              ...
              src={user?.photoURL ? IMAGE_PROXY(user.photoURL) : DEFAULT_AVATAR}
              alt=""
            />
            ...
            {conversation.group?.admins?.includes(currentUser?.uid as string) &&
              user.uid !== currentUser?.uid && (
                <div className="group relative flex-shrink-0" tabIndex={0}>
                  ...
                  <div className="bg-lighten border-dark-lighten invisible absolute top-full right-0 z-[1] flex w-max flex-col items-stretch rounded-lg border py-1 opacity-0 transition-all duration-300 group-focus-within:!visible group-focus-within:!opacity-100">
                    <button
