          ...
          .map((user) => (
            <div key={user.uid} className="flex items-center gap-3 px-4">
              ...
              <div className="flex-grow">
                <h1 className="text-dark-lighten">
              ...
              {conversation.group?.admins?.includes(
                ...
                <div className="group relative flex-shrink-0" tabIndex={0}>
                  <button>
                    <i className="bx bx-dots-horizontal-rounded text-dark-lighten text-2xl"></i>
                  </button>

                  <div className="bg-lighten border-lighten invisible absolute top-full right-0 z-[1] flex w-max flex-col items-stretch rounded-lg border py-1 opacity-0 transition-all duration-300 group-focus-within:!visible group-focus-within:!opacity-100">
                    {conversation.users.length > 3 && (
                      <button
                        onClick={() => handleRemoveFromGroup(user.uid)}
                        className="bg-lighten flex items-center gap-1 px-3 py-1 transition duration-300 hover:brightness-125"
                      >
                        <i className="bx bx-user-x text-dark-lighten text-2xl"></i>
                    ...
                    {user.uid !== currentUser?.uid && (
                      <button
                        ...
                        className="bg-lighten flex items-center gap-1 px-3 py-1 transition duration-300 hover:brightness-125"
                      >
                        <i className="bx bx-user-check text-dark-lighten text-2xl"></i>
                        <span className="text-dark-lighten">Make an admin</span>
                      </button>
                    )}
                  </div>
                </div>
